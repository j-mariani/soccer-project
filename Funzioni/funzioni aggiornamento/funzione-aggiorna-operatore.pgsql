create or replace function aggiorna_operatore(nome_utente varchar(20),tipo_operazione integer, password varchar(20),nuovo_nome_utente varchar(20) default null) returns integer AS $$
declare
    num1 integer;
...
begin
    ...
    if(tipo_operazione = 1) then
        update operatori set 
            password=coalesce(pass,operatori.password)
        where operatori.nome_utente=nu;
        ...
        if EXISTS (select coalesce from coalesce(pass) where coalesce is not null) then
            if(ck = nnu) then
                update operatori set
                    nome_utente = coalesce(ck,operatori.nome_utente),
                    password = coalesce(pass,operatori.password)
                where operatori.nome_utente=nu;
                GET DIAGNOSTICS num1 = ROW_COUNT;
